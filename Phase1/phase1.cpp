...
private:
    char buffer[40];
    char Memory[100][4];
    char IR[4];  // Instruction Register sa, instructions will store
    char R[4];   // General purpose Register, content of memory will store
        ...
        if (infile.is_open())
        {
            ...
            { 
                if (s[0] == '$' && s[1] == 'A' && s[2] == 'M' && s[3] == 'J'){
                    ...
                    int buff = 0; // Represent the index of program data card string
...
void READ()
{
    ...
    int buff = 0; // Here it will act as, index of Input file data
    
    while (buff < len && mem_ptr < 100) {
        // Copy a chunk of data from data to Memory
        memcpy(Memory[mem_ptr], &data[buff], min(4, len - buff));
        
        buff += 4;

        mem_ptr++; // Going to next memory location
    }
